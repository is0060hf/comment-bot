...
describe('Policies Integration', () => {
  ...
  describe('Comment processing pipeline', () => {
    ...
    it('should handle edge case: very long comment with NG words and emojis', () => {
      let comment =
        'この配信者はアホみたいなことばかり言っていて本当に困りますね😡💢😤もっとまともな内容にしてほしいです！';
    ...
    it('should handle normalization edge cases', () => {
      ...
      for (const variant of variations) {
        ...
        // デバッグ情報
        const normalized = ngWordsPolicy.normalize(variant);
        console.log(`Original: "${variant}", Normalized: "${normalized}"`);
        if (result.isValid) {
          console.log(`Failed to detect NG word in: "${variant}", detected: ${result.detectedWords.join(', ')}`);
        }

        expect(result.isValid).toBe(false);
        // 検出されたNG語は「バカ」
        expect(result.detectedWords.some(w => w.includes('バカ') || w.includes('ばか'))).toBe(true);
