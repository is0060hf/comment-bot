...
import { YouTubePort, LiveChatMessage, YouTubeError, RateLimitInfo } from '../../../src/ports/youtube';
...
describe('MockYouTubeAdapter', () => {
  ...
  describe('postMessage', () => {
    ...
    it('should enforce character limit', async () => {
      ...
      await expect(adapter.postMessage(liveChatId, longMessage))
    ...
    it('should simulate random failures', async () => {
      ...
      await expect(failingAdapter.postMessage('chat-id', 'message'))
        .rejects.toThrow(YouTubeError);
    });
  });
  ...
  describe('getLiveChatId', () => {
    ...
    it('should throw error for non-live video', async () => {
      ...
      await expect(nonLiveAdapter.getLiveChatId('video-id'))
        .rejects.toThrow(YouTubeError);
    });
  });
