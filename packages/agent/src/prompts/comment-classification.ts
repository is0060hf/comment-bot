...
例:
{
   ...
   * 最近のコメントをフォーマット
   */
  private formatRecentComments(recentComments?: CommentClassificationContext['recentComments']): string {
    ...
    return `最近のコメント傾向:
${recentComments.slice(0, 3).map(c => `- ${c.message}`).join('\n')}
${similarityWarning}`;
  }
   ...
   * 類似コメントをチェック
   */
  private checkForSimilarComments(recentComments: CommentClassificationContext['recentComments']): string {
...
- テンションの高い反応を優先`
    ...
    const tone = this.config.tone as keyof typeof toneRules;
