...
- STTプロバイダ（ローカル）: GCP/Azure/Deepgram/Whisper API（選択式）
- モデレーション（ローカル）: Azure AI Content Safety / OpenAI Moderation（選択式）
- 設定同期（任意）: Vercel（Edge Config / KV等）を介したサーバーサイドの設定提供。秘密情報・トークンは保管しない

認証/鍵管理: YouTube OAuthはローカルエージェントで実施（トークンはローカルのみ保持）。Web UIでは秘密を保存しない。
...
- スキーマ: `要件定義書.md` 記載（app/platforms/audio/stt/llm/persona/contentPolicy/triggerPolicy/safety/logging/telemetry）
...
- **YouTube OAuth/権限（要確認含む）**
  ...
  - 自動リフレッシュ対応（期限切れ時の更新）
  - 必要スコープ（要確認）: `https://www.googleapis.com/auth/youtube.force-ssl` もしくは `https://www.googleapis.com/auth/youtube`
...
- **安全フィルタ**
  - プロバイダ（Azure/OpenAI）とカテゴリしきい値
  - PII検出時の伏字/抑止、失敗時のルールベース代替
- **YouTube投稿（A案: ローカル直接）**
  - `liveChatMessages.insert`の実装
  - メッセージ長の上限に収める（要確認: YouTube Live Chat仕様）
  - バックオフ（指数）、重複/類似投稿の抑止
- **スケジューラ/レート制御**
  - `minIntervalSeconds`, `maxPer10Min`, `cooldownSeconds`, `dedupeWindowSec`
  - 同義反復の距離指標（簡易類似度）
- **設定・同期（Next/Vercel）**
