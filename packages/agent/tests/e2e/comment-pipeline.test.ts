...
describe('Comment Pipeline E2E', () => {
  ...
  describe('エラーハンドリング', () => {
    ...
    test('YouTube API レート制限の処理', async () => {
      ...
      jest
        .spyOn(limitedYoutubeAdapter, 'postMessage')
        .mockImplementation(async (liveChatId, message) => {
          if (postCount >= 2) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, config.rateLimit.minIntervalSeconds * 1000 + 100)
      );
      ...
      const testCases = [
        {
          name: '長すぎるコメント',
          mockTranscript:
            'これは非常に長いコメントで、文字数制限を超えてしまう可能性があるため、適切に短縮される必要があります',
